...
struct selection_sorter:
    cppsort::sorter_facade<selection_sorter>
{
    using cppsort::sorter_facade<selection_sorter>::operator();
...
In the previous example, [`sorter_facade`](sorter-facade.md) is a CRTP base class
that is used to provide the function pointer conversion operators to the sorter. It
also generates the range overloads of `operator()`. The `using` declaration is needed
if you don't want the `sorter_facade` overloads of `operator()` to be hidden by the
one you defined.

Note that you can still provide `operator()` overloads equivalent to those provided
by `sorter_facade`. For example, if the sorting algorithm you are wrapping allows an
...
sorter, and it should properly hide the corresponding overload in `sorter_facade`:
...
struct counting_sorter:
    cppsort::sorter_facade<counting_sorter>
{
    using cppsort::sorter_facade<counting_sorter>::operator();
...
The appropriate operations will still be generated by `sorter_facade` and you should
not experiment a problem until you try to feed a comparison function to this sorter.
That being said, non-comparison sorts tend to work only with collections of some
specific types since they generally use the properties of these types to sort the
collection. For example, our `counting_sorter` only works with built-in integer types,
but this information currently does not appear anywhere in the interface.
...
functions. To support that model, [`sorter_facade`](sorter-facade.md) adds overloads to
...
overload for `std::greater<T>`, but `sorter_facade` does not automate the generation
